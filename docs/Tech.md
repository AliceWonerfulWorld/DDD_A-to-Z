...
| 負荷・耐久性検証 | Dlang | 許可された環境に対する defensive load testing CLI |
...
| C | C++ | コアロジック（WASM経由） |
| D | Dlang | defensive load testing CLI |
| E | Elixir | リアルタイムWebSocket通知 |
| F | FrankenPHP | Admin サービスの本番ランタイム（Docker / Cloud Run） |
| G | Go | MVPバックエンド / 将来のBFF・worker |
| H | Haskell | 勢力計算ロジックサービス |
| I | Inertia.js | Admin サービスのサーバー↔Vue橋渡し |
...
| P | PHP / Laravel | Admin サービスのWebフレームワーク（マスターデータ管理画面） |
